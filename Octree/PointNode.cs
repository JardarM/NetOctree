...
namespace Octree
{
    ...
    public class PointNode<T>
    {
        ...
        /// Child nodes, if any
        /// </summary>
        private PointNode<T>[] _children = null;
        ...
        public PointNode(float baseLengthVal, float minSizeVal, Vector3 centerVal)
        {
        ...
        public void SetChildren(PointNode<T>[] childOctrees)
        {
        ...
        public PointNode<T> ShrinkIfPossible(float minLength)
        {
        ...
        /// </summary>
        /// <param name="ray">The ray.</param>
        /// <param name="vector3">The point to check distance from the ray.</param>
        /// <returns>Squared distance from the point to the closest point of the ray.</returns>
        public static float SqrDistanceToRay(Ray ray, Vector3 vector3)
        {
            return Vector3.Cross(ray.Direction, vector3 - ray.Origin).LengthSquared();
        }
        ...
        private void Split()
        {
            ...
            _children = new PointNode<T>[8];
            _children[0] = new PointNode<T>(newLength, _minSize, Center + new Vector3(-quarter, quarter, -quarter));
            _children[1] = new PointNode<T>(newLength, _minSize, Center + new Vector3(quarter, quarter, -quarter));
            _children[2] = new PointNode<T>(newLength, _minSize, Center + new Vector3(-quarter, quarter, quarter));
            _children[3] = new PointNode<T>(newLength, _minSize, Center + new Vector3(quarter, quarter, quarter));
            _children[4] = new PointNode<T>(newLength, _minSize, Center + new Vector3(-quarter, -quarter, -quarter));
            _children[5] = new PointNode<T>(newLength, _minSize, Center + new Vector3(quarter, -quarter, -quarter));
            _children[6] = new PointNode<T>(newLength, _minSize, Center + new Vector3(-quarter, -quarter, quarter));
            _children[7] = new PointNode<T>(newLength, _minSize, Center + new Vector3(quarter, -quarter, quarter));
        }
        ...
        private void Merge()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                PointNode<T> curChild = _children[i];
        ...
        private bool ShouldMerge()
        {
            ...
            if (_children != null)
            {
                foreach (PointNode<T> child in _children)
                {
